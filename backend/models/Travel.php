...
class Travel extends \yii\db\ActiveRecord
{
    ...
    public function doadd($t_p_img)
    {
        ...
        $c_id = $_POST['c_id'];
        $s_id = $_POST['s_id']; 
        $sql = "insert into travel(t_name,t_content,t_j,t_w,c_id,s_id,t_p_img,t_del) values('$t_name','$t_content','$t_j','$t_w','$c_id','$s_id','$t_p_img',1)";      
    ...
    public function dosave($t_p_img)
    {
        ...
        //判断是否修改图片
        if ($t_p_img) {   
            $upda = Yii::$app->db->createCommand()->update('travel',['t_name'=>$t_name,'t_p_img'=>$t_p_img,'t_content'=>$t_content,'t_j'=>$t_j,'t_w'=>$t_w,'c_id'=>$c_id,'s_id'=>$s_id,'t_money'=>$t_money],"t_id='$id'")->query();
        }else{
            $upda = Yii::$app->db->createCommand()->update('travel',['t_name'=>$t_name,'t_content'=>$t_content,'t_j'=>$t_j,'t_w'=>$t_w,'c_id'=>$c_id,'s_id'=>$s_id,'t_money'=>$t_money],"t_id='$id'")->query();
